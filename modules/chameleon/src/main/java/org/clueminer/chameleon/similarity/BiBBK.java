...
public class BiBBK<E extends Instance> extends AbstractSimilarity<E> implements MergeEvaluation<E> {

    public static final String name = "BiBBK";
    ...
    public double score(Cluster<E> a, Cluster<E> b, Props params) {
        ...
        int b1 = x.getBiEdgeCount();
        int b2 = y.getBiEdgeCount();
        double biRatio = 1.0;
        if (b1 > 0) {
            biRatio += b1 / ec1 * 10;
        }
        ...
        double val = biRatio * (gps.getCnt(i, j) / (Math.min(ec1, ec2)))
