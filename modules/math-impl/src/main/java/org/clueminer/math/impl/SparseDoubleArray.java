 ...
 *
 * The {@code get} operation runs in logarithmic time. The {@code set} operation
 * runs in consant time if setting an existing non-zero value to a non-zero
 * value. However, if the {@code set} invocation sets a zero value to non-zero,
 * the operation is linear with the size of the array.<p>
 *
...
public class SparseDoubleArray
    ...
    /**
     * Creates a sparse {@code double} array that grows to the maximum size set
     ...
     *
     * @throws ArrayIndexOutOfBoundException if the index is greater than the
