...
public class EvaluatorPlot<E extends Instance, C extends Cluster<E>> extends JPanel {
    ...
    private DrawablePanel clusteringPlot(Collection<? extends Clustering> clusterings) {
        ...
        clusterings.stream().map(clust -> evaluationTable(clust)).forEachOrdered(table -> {
        ...
        List<PointRenderer> renderers = plot.getPointRenderers(ds);
        if (renderers.size() > 0) {
            PointRenderer pointRenderer = renderers.get(0);
            orig = Color.BLUE;
            ...
            // Format plot
            //plot.setInsets(new Insets2D(20.0, 40.0, 40.0, 40.0));
            //plot.getTitle().setText(clustering.getName());
            plot.setLegendVisible(false);

            // Format axes
            AxisRenderer axisRendererX = plot.getAxisRenderer(XYPlot.AXIS_X);
            axisRendererX.setLabel(new Label(evaluatorX.getName()));
            AxisRenderer axisRendererY = plot.getAxisRenderer(XYPlot.AXIS_Y);
            axisRendererY.setLabel(new Label(evaluatorY.getName()));
        }
