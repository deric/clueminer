 ...
 * panning to control the associated {@link Navigable} object. At the moment the
 * only supported operations are zooming and panning.
 *
 ...
 * implemented with the methods {@link #setDefaultState()} and {@link #reset()}.
 *
 ...
 * horizontal or vertical—by the convenience methods {@link #getDirection()} and
 * {@link #setDirection(NavigationDirection)}. The data type, e.g. an enum type,
 * for directions must implement the interface {@link NavigationDirection}.
 *
 * Sometimes, actions performed on an object should be applied to another object
 * synchronously. The methods {@link #connect(Navigator)} and
...
public interface Navigator extends NavigationListener {
     ...
     *
     * @return {@code true} if the object can be zoomed, {@code false}
     *         otherwise.
     */
    boolean isZoomable();
    ...
    /**
     * Increases the current zoom level by the specified zoom factor. The zoom
     * will only be changed if the navigator is zoomable.
     *
     * @see #isZoomable()
    ...
    /**
     * Decreases the current zoom level by the specified zoom factor. The zoom
     * will only be changed if the navigator is zoomable.
     *
     * @see #isZoomable()
     * @see #setZoomable(boolean)
    ...
    /**
     * Scale the associated object at the specified point. If zooming is
     * disabled nothing will be done. If panning is disabled zooming will be
     * applied around the current center.
     *
     * @param zoom      New zoom level.
     ...
     *
     * @return {@code true} if the object can be panned, {@code false}
     *         otherwise.
     */
    boolean isPannable();
    ...
    /**
     * Sets a new center point. The values of the point are in world units. The
     * center point will only be changed if the navigator is pannable.
     *
    ...
    /**
     * Moves the center by the relative values of the specified point. The
     * values of the point are in screen units. The center point will only be
     * changed if the navigator is pannable.
     *
    ...
    /**
     * Sets the current state as the default state of the object. Resetting the
     * navigator will then return to the default state.
     */
    ...
    /**
     * Returns the factor which is used to change the zoom level on zoom in/out
     * actions.
     *
    ...
    /**
     * Sets the factor which should be used to change the zoom level on zoom
     * in/out actions.
     *
