...
public class KMeansPPSelection extends AbstractSelection implements SeedSelection {
    ...
    public int[] selectIntIndices(final Dataset<? extends Instance> dataset, int k, ExecutorService threadpool) {
        int[] indices = new int[k];
        //Initial random point
        indices[0] = rand.nextInt(dataset.size());
        ...
        List<Future<Double>> futureChanges = new ArrayList<Future<Double>>(SystemInfo.LogicalCores);

        for (int j = 1; j < k; j++) {
            //Compute the distance from each data point to the closest mean
            final Vector newMean = dataset.get(indices[j - 1]);//Only the most recently added mean needs to get distances computed.
            futureChanges.clear();
