...
public class FivePLOverlay extends AbstractOverlay implements Overlay {
    ...
    protected double[] fetchParams(int idx) {
        double[] params = new double[approximator.getNumCoefficients()];
        int i = 0;
        for (String p : approximator.getParamNames()) {
            params[i++] = approxData.getAttributeValue(p, idx);
        }
        return params;
    }
