...
public class SAXTrieTree {
    ...
    public List<Integer> getOccurrences(String string) throws TrieException {
        ...
        if (this.root.getDescendants().size() > 0 && string.length() > 0
            ...
            //
            if (2 == string.length()) {
                return ((TrieLeafNode) ((TrieInnerNode) this.root.getDescendant(string.substring(0, 1))).getDescendant(string.substring(1))).getOccurences();
            }
     ...
     *
     * @param root The current node, IT IS NOT ROOT NODE, we just call it root
     * here due to context.
