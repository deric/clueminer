...
public class SlopeOverlay extends AbstractOverlay implements Overlay {
    ...
    public void paint(Graphics2D g, ChartConfig cf, Rectangle bounds) {
        ...
        if (dataset != null) {
            ...
            for (ContinuousInstance inst : ts) {
                ...
                System.out.println("std dev = " + inst.getStdDev() + " min " + inst.value(minIdx) + " max " + inst.value(maxIdx));
                if (Math.abs(inst.value(maxIdx) - inst.value(minIdx)) < inst.getStdDev()) {
                    minIdx = 0;
                    maxIdx = 0;
                }
                drawStdDev(g, cf, inst.value(minIdx) + inst.getStdDev(), inst);
    ...
    private void drawStdDev(Graphics2D g, ChartConfig cf, double stdY, Instance inst) {
        ...
        double y = cd.getY(stdY, rect, range);
        double x1 = cd.getX(0, rect);
        double x2 = cd.getX(inst.size() - 1, rect);
